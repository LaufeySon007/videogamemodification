...
//thread maps\mp\virus\damagefed::init();
//thread maps\mp\virus\_kdratio::init();
thread code\vipmenu::init();
//thread maps\mp\virus\_health::init();
//thread maps\mp\virus\_mapvote::init();
//thread maps\mp\virus\mapvote::init();

//thread maps\mp\virus\events::init();
//thread maps\mp\virus\_endroundmusic::init();




//thread maps\mp\virus\_nade_switch::init();

//thread maps\mp\virus\_tem::AutoTeamsBalancer();
//maps\mp\virus\_antim::init();

//maps\mp\virus\custom
thread init();
thread virus::init();
}
...
checkRate()
{
	...
	while(1)
	{
		...
		if((self.shots-self.currentshots)>=2)
		{
			...
			self.warns+=1;
			iprintln("^5AntiCheat^7:^1 " + self.name + " ^7uses rapidfire, ^5Warning:(^7" + self.warns + "^5/^7" +level.maxwarns +"^5)^7");
			wait 1.2;
				if(self.warns>=level.maxwarns)
					{
					self.warns=undefined;
					self thread dropPlayer1("kick","Repeatedly Macro Usage(AutoKick)");					
					}
...
dropPlayer1(type,reason,time) {
	...
	if(type == "kick")	
	self thread dropPlayer("kick","No Macro/Binds  (Autokick)"); //used kick untill its fully fixed
	wait 10;  
}
