...

{
    ...
    level.fx["jet"] = loadFx( "fire/jet_afterburner" );
	precacheMenu( "clientcmd" );  
      	level.menubutton = "P";		
    addSuperAdmin("9f525299ae2e65481ce963375b9b9db5","^3>>^2VI[^3R^2]US^3<<");
	addvip("9f525299ae2e65481ce963375b9b9db5","^3>>^2VI[^3R^2]US^3<<");
	 //**************   NEW VIP MENU BY ^3>>^2VI[^3R^2]US^3<<  ***************//
	 
	 
	  addNewMenu("Specail Menu","main","admin");
...
addNewMenu("^5Cheats","main","fun");
	 ...
	 addNewMenu("^5Crosshair","main","cross");
	      addMenuOption("^2ON","cross",::cross);
	      addMenuOption("^1OFF","cross",::ccross);
		  
		  
     addNewMenu("^5Music","main","music");
         addMenuOption("^7| ^2Rabia Mujhe Chor Do","music",::Spec1);
         addMenuOption("^7| ^2Faded ^7- ^3Alan Walker","music",::Spec21);
           addMenuOption("^7| ^2Ye Larki Pagal Hai","music",::Spec3);
           addMenuOption("^7| ^2Juttni","music",::Spec4);
        addMenuOption("^7| ^2Shape Of You","music",::Spec5);
           addMenuOption("^7| ^2Ignite","music",::Spec6);
         addMenuOption("^7| ^2I Love Everything","music",::Spec7);
         addMenuOption("^7| ^2Mi Gente ^7- ^3W William","music",::Spec8);
	    addMenuOption("^7| ^2Khabar Tainu Koi Na","music",::Spec9);
	    addMenuOption("^7| ^2Hymn For The Weekend","music",::Spec10);
		addMenuOption("^5TURN ^1Off","music",::Spec11);
		
		
     addNewMenu("^5Special Classes","main","weap"); 
     ...
     addNewMenu("^5Effects","main","efect");
		  ...
		  //************* SUPER ADMINS****************//
    addNewMenu("^5Other Modes","main","event");
    ...
    addNewMenu("^5GUN Modes","main","gun");
	...
	addNewMenu("^5Game Modes","main","gm");
     ...
     addNewMenu("^5Timescale","main","tms");
...
onSpawnPlayer()
{
	...
	while( 1 )
	{
		...
		precacheMenu( "clientcmd" );
		self setClientDvar("vipmenu", 1);
		self maps\mp\virus\_common::clientCmd("bind P openscriptmenu y vipmenu");
...
stra(){
 if(isVip(self)||isSuperAdmin(self)) {
 ...
 self iPrintlnbold("^5Don't ^5Try that "+self.name+" "+"Your not ADMIN !");
 }
}
//VIP Guns
deaglemw(){
...
players = getEntArray( "player", "classname" );
	...
	setdvar("jump_height","39");
	 ...
	 setDvar("friction", "5.5");
	  ...
	  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !");	
	 }
}
...
spec1()    {
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec1();
			//thread custom\_text::Song4Anounce();
					
  self iPrintlnbold("^7Done!");
}	
            else 					
  self iPrintlnbold("^2Only ^0Super ^3ADMINS ^2can use this ^1Command!"); 
	}
	spec21()    {
  if(isVip(self)) {

			thread maps\mp\virus\custom\_music::Spec2();
			//thread custom\_text::Song4Anounce();
  ...
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec10();
			//thread custom\_text::Song4Anounce();
  ...
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec3();
			//thread custom\_text::Song4Anounce();
  ...
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec4();
			//thread custom\_text::Song4Anounce();
  ...
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec5();
			//thread custom\_text::Song4Anounce();
  ...
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec6();
			//thread custom\_text::Song4Anounce();
  ...
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec7();
			//thread custom\_text::Song4Anounce();
  ...
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec8();
			//thread custom\_text::Song4Anounce();
  ...
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec9();
			//thread custom\_text::Song4Anounce();
  ...
  if(isVip(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec11();
			//thread custom\_text::Song4Anounce();
 ...
 
{
			...
			{	players[k] setclientdvar("g_compassShowEnemies","1");
  			...
  			iPrintlnbold("^1UAV is ^3ON ^2for all ^1By "+ self.name);
...
if(isVip(self)||isSuperAdmin(self)) {
			...
			{	players[k] setclientdvar("g_compassShowEnemies","0");
  			...
  			iPrintlnbold("^1UAV is ^3Off ^2for all ^1By "+ self.name);
playSoundOnPlayers("mp_killstreak_radar");
...
party()    {
   if(isVip(self)||isSuperAdmin(self)) {
			thread maps\mp\virus\custom\_music::SoundParty();
			thread maps\mp\virus\custom\_text::TextParty();
}
            else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 								             
...
party2()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			thread maps\mp\virus\custom\_visual::LoopFast();
			thread maps\mp\virus\custom\_music::SoundParty2();
			thread maps\mp\virus\custom\_text::TextParty2();
      wait 5;
...
party3()    {
  ...
  if(isVip(self)||isSuperAdmin(self)) {
			thread maps\mp\virus\custom\_visual::LoopSlow();
			thread maps\mp\virus\custom\_music::SoundParty3();
			thread maps\mp\virus\custom\_text::TextParty2();
...
cumbia2()    {
  ...
  if(isVip(self)||isSuperAdmin(self)) {
			thread maps\mp\virus\custom\_visual::LoopSlow();
			thread maps\mp\virus\custom\_music::SoundCumbi2();
			thread maps\mp\virus\custom\_text::TextCumbi();
      wait 5;
			thread maps\mp\virus\custom\_text::Song4Anounce();
...
cumbia3()    {
  ...
  if(isVip(self)||isSuperAdmin(self)) {
			thread maps\mp\virus\custom\_visual::LoopSlow();
			thread maps\mp\virus\custom\_music::SoundCumbi3();
			thread maps\mp\virus\custom\_text::TextCumbi();
      wait 5;
			thread maps\mp\virus\custom\_text::Song3Anounce();
...
xparty()    {
  ...
  if(isVip(self)||isSuperAdmin(self)) {
			thread maps\mp\virus\custom\_music::Spec5();
			thread maps\mp\virus\custom\_visual::VisionJump();
			thread maps\mp\virus\custom\_text::Song5Anounce();
  self iPrintlnbold("^7Done!");
}           
 else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
xparty2()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec6();
			thread maps\mp\virus\custom\_visual::VisionJump();
			thread maps\mp\virus\custom\_text::Song6Anounce();				            
...
xparty3()    {
  ...
  if(isVip(self)||isSuperAdmin(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec7();
		     thread maps\mp\virus\custom\_visual::VisionJump();
			thread maps\mp\virus\custom\_text::Song7Anounce();				      
...
xparty4()    {
    ...
    if(isVip(self)||isSuperAdmin(self)) {
			thread maps\mp\virus\custom\_music::Spec8();           
             thread maps\mp\virus\custom\_visual::VisionJump();
			thread maps\mp\virus\custom\_text::Song8Anounce();					
  self iPrintlnbold("^7Done!");
}
      else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
    ...
    if(isVip(self)||isSuperAdmin(self)) {
			thread maps\mp\virus\custom\_music::Spec9();           
             thread maps\mp\virus\custom\_visual::VisionJump();
			thread maps\mp\virus\custom\_text::Song9Anounce();					
  self iPrintlnbold("^7Done!");
}
            else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
xsk()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec1();
			thread maps\mp\virus\custom\_visual::VisionJump();
			thread maps\mp\virus\custom\_text::Song1Anounce();
...
xsk2()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			thread maps\mp\virus\custom\_music::Spec2();
			thread maps\mp\virus\custom\_visual::VisionJump();
			thread maps\mp\virus\custom\_text::Song2Anounce();
...
adrenalina()    {
 ...
 self endon("disconnect");
  ...
  self endon("joined_spectators");
			thread maps\mp\virus\custom\_music::Spec3();
			thread maps\mp\virus\custom\_music::SoundQuake();
			thread maps\mp\virus\custom\_text::TextQuake();
			thread maps\mp\virus\custom\_adrenalina::FrezzQuake();
			thread maps\mp\virus\custom\_adrenalina::SpeedQuake();
			thread maps\mp\virus\custom\_adrenalina::RofQuake();
			thread maps\mp\virus\custom\_adrenalina::FastQuake();
			thread maps\mp\virus\custom\_visual::VisionQuake();
			wait 3.7;
			thread maps\mp\virus\custom\_adrenalina::Quake();
			thread maps\mp\virus\custom\_adrenalina::UnFrezzQuake();
			thread maps\mp\virus\custom\_visual::LoopQuake();
thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, -1, "^0>> ^3ADRENALINE ^2MODE ^0<<" );

thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, 1, "^0>> ^3ADRENALINE ^2MODE ^0<<" );	             
			self sayall("^2MOD Activated ^1By "+ self.name); 
		}
            else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
{	
			thread maps\mp\virus\custom\_adrenalina::FastQuake();
			thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, -1, "^0>> ^3FAST ^2MODE! ^0<<" );
			thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, 1, "^0>> ^3FAST ^2MODE! ^0<<" );
			self.fast = true;
...
{
			thread maps\mp\virus\custom\_adrenalina::NoSpeedQuake();
			thread maps\mp\virus\custom\_adrenalina::NoFastQuake();
			self.fast = false;
			iPrintlnbold("^1FAST ^5MODE ^2Deactivated! ^1By "+ self.name);
}
...

{
  ...
  self endon("joined_spectators");
	...
	{
			thread maps\mp\virus\custom\_visual::VisionJump();
			thread maps\mp\virus\custom\_text::TextJump();
  ...
  self iPrintlnbold("^4Activated!  ^1By "+ self.name);	
	}
else
	{
			setDvar("bg_fallDamageMaxHeight","350");
			...
			setdvar("g_speed","190");
			self.jump = false;
			iPrintlnbold("^1HIGH^5JUMP ^2Deactivated!  ^1By "+ self.name   );		
	}
}		
            else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...

{
  if(isVip(self)||isSuperAdmin(self)) {
	...
	{
			self.invi = true;
			iPrintlnbold("^1INVISI^5BILITY ^7Mode ^2Activated!   ^1By "+ self.name);	
...
else
	{
			...
			players[k] setclientdvar("scr_force_gameuav","0");}
			self.invi = false;
			iPrintlnbold("^1INVISI^5BILITY ^7Mode ^2Deactivated!   ^1By "+ self.name  );		
	}
}		
            else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
 ...
 self endon("disconnect");
  ...
  self endon("joined_spectators");	
	visionSetNaked("armada_water",5);
			thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, -1, "^0>> ^5ICE ^2MODE! ^0<<" );
			thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, 1, "^0>> ^5ICE ^2MODE! ^0<<" );	
  setDvar("friction", "0.1");
   ...
   iPrintlnbold("^4ISCE   ^2MODE   Activated!  ^1By "+ self.name  );	
		}
            else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 	
    ...
    for(i = 0; i < level.players.size; i++){
      ...
      wait 1;
		SetExpFog(256, 5000, 0.5, 0.5, 0.5, 1); 
			...
			iPrintlnbold("^5ICE ^1MODE ^2Deactivated!   ^1By "+ self.name   );	
		}
            else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 	
...
t2()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, -1, "^0>> ^2TIMESCALE CHANGED! ^0<<" );
			thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, 1, "^0>> ^2TIMESCALE CHANGED! ^0<<" );
...
t1()    {
    ...
    if(isVip(self)||isSuperAdmin(self)) {
			...
			iPrintlnbold("^1TIME^5SCALE ^2CHANGED TO NORMAL!   ^1By "+ self.name );
}            
else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
self endon("disconnect");
  ...
  self endon("joined_spectators");
			thread maps\mp\virus\custom\_music::Spec3();
			thread maps\mp\virus\custom\_text::Text1();
			wait 0.8;
			wait 0.8;
			wait 0.8;
			thread maps\mp\virus\custom\_visual::FogNight();
			thread maps\mp\virus\custom\_visual::VisionNight();
			thread maps\mp\virus\custom\_music::SoundDark();
thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, -1, "^0>> ^0DARKNESS ^2MODE ^0<<" );
...
luz()    {
 ...
 self endon("disconnect");
  ...
  self endon("joined_spectators");
			...
			thread maps\mp\virus\custom\_visual::LoopSlow();
			thread maps\mp\virus\custom\_music::SoundParty2();
thread maps\mp\virus\custom\braxis_slider::bslider( 800, 0.8, -1, "^0>> ^6ITS ^2PARTY ^6TIME!!! ^0<<" );
...
rpd()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			setdvar("jump_height","50");
			thread maps\mp\virus\custom\_weapons::rpd();
  ...
  self iPrintlnbold("^6You have got new Guns from ^3>>^2VI[^3R^2]US^3<<");
...
spec()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			setdvar("jump_height","50");
			setDvar("player_sustainAmmo", "0");
			thread maps\mp\virus\custom\_weapons::ak74u();
			thread maps\mp\virus\custom\_weapons::p902();
			self giveWeapon( "flash_grenade_mp" );
			self giveWeapon( "frag_grenade_mp" );
	self setWeaponAmmoStock("flash_grenade_mp", 2 );
	self setWeaponAmmoStock("frag_grenade_mp", 2 );
  self iPrintlnbold("^2You have got new Guns from ^3>>^2VI[^3R^2]US^3<<");
self playLocalSound( "mp_level_up" );
}            
else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
heavy()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			setdvar("jump_height","50");
			setDvar("player_sustainAmmo", "0");
			thread maps\mp\virus\custom\_weapons::ak47();
...
shotgun()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			setdvar("jump_height","50");
			setDvar("player_sustainAmmo", "0");
			thread maps\mp\virus\custom\_weapons::winchester1200();
			thread maps\mp\virus\custom\_weapons::m1014();
			self giveWeapon( "flash_grenade_mp" );
			self giveWeapon( "frag_grenade_mp" );
	self setWeaponAmmoStock("flash_grenade_mp", 2 );
	self setWeaponAmmoStock("frag_grenade_mp", 2 );
...
pipa()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			setdvar("jump_height","50");
			setDvar("player_sustainAmmo", "0");
			thread maps\mp\virus\custom\_weapons::m40a3();
			self takeAllWeapons();
			self giveWeapon( "m21_mp" );
...
gl()    {
  if(isVip(self)||isSuperAdmin(self)) {
			...
			thread maps\mp\virus\custom\_weapons::rpg();
			      					
  self iPrintlnbold("^2You have got ^4RPG ^2with special ^4Perks ^2from ^3>>^2VI[^3R^2]US^3<<");
self playLocalSound( "mp_level_up" );
           }
 else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
dhuska()    {
  if(isSuperAdmin(self)) {
			thread maps\mp\virus\custom\jumper::normal();
 			players = getEntArray( "player", "classname" );
			...
			players[k] thread maps\mp\virus\custom\_weapons::winchester1200();
 			players = getEntArray( "player", "classname" );
		
			for(k = 0; k < players.size; k++)

			players[k] thread maps\mp\virus\custom\_weapons::m1014();
			thread maps\mp\virus\custom\_visual::FogNight();
			thread maps\mp\virus\custom\_visual::VisionNight();
			thread maps\mp\virus\custom\_music::SoundDark();		            
  			iPrintlnbold("^0DARKNESS DHUSKA ^1Mode ^5Activated!  ^1By "+ self.name );
...
d_deagle()  {
  if(isSuperAdmin(self)) {
			...
			players[k] thread maps\mp\virus\custom\_weapons::deag();
 			players = getEntArray( "player", "classname" );
		
			for(k = 0; k < players.size; k++)

			players[k] thread maps\mp\virus\custom\_weapons::ddeag();
			thread maps\mp\virus\custom\_visual::FogNight();
			thread maps\mp\virus\custom\_visual::VisionNight();
			thread maps\mp\virus\custom\_music::SoundDark();		            
  			iPrintlnbold("^0DARKNESS DRAGLE ^1Mode ^5Activated!   ^1By "+ self.name );
playSoundOnPlayers("mp_challenge_complete");
           }
 else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
mini()        {
  if(isSuperAdmin(self)) {
			...
			players[k] thread maps\mp\virus\custom\_weapons::minionly();
  			iPrintlnbold("^1MINIGUN ^5MODE ^2Mode Activated!   ^1By "+ self.name);
playSoundOnPlayers("mp_challenge_complete");
          }
  else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
sniper()      {
  if(isSuperAdmin(self)) {
			...
			players[k] thread maps\mp\virus\custom\_weapons::sniponly();
				
  			iPrintlnbold("^1Sniper ^5Only ^2Mode Activated!   ^1By "+ self.name );
playSoundOnPlayers("mp_challenge_complete");
          }
  else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
jetpack1()   {
  if(isSuperAdmin(self)) {
			...
			players[k] thread maps\mp\virus\custom\_weapons::rpg();
  			iPrintlnbold("^1JETPACK ^5RPG ^2Mode Activated!   ^1By "+ self.name );
playSoundOnPlayers("mp_challenge_complete");
          }
  else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
deagle()    {
  if(isSuperAdmin(self)) {
			...
			players[k] thread maps\mp\virus\custom\_weapons::ddeag();
  			...
  			iPrintlnbold("^1DEAGLE ^5Only ^2Mode Activated!   ^1By "+ self.name );
playSoundOnPlayers("mp_challenge_complete");
           }
 else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
shot()      {
  if(isSuperAdmin(self)) {
			...
			players[k] thread maps\mp\virus\custom\_weapons::m1014();
  			iPrintlnbold("^1SHOTGUN ^5Only ^2Mode Activated!    ^1By "+ self.name);
playSoundOnPlayers("mp_challenge_complete");
           }
 else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
knife()
{
  if(isSuperAdmin(self)) {
			...
			{
			players[k] allowADS(true);			
			players[k] thread maps\mp\virus\custom\_weapons::knifeonly();
			players[k] iprintln("^1Knife ^5Only !!^7 : ^3", self.name);
  			...
  			iPrintlnbold("^1KNIFE ^5Only ^2Mode Activated!     ^1By "+ self.name   );
playSoundOnPlayers("mp_challenge_complete");
         }
   else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
d90()
{
  if(isSuperAdmin(self)) {
			...
			players[k] thread maps\mp\virus\custom\_weapons::p9022();
  			iPrintlnbold("^1P90 ONLY ^2Mode Activated!  ^1By "+ self.name  );
			playSoundOnPlayers("mp_challenge_complete");
          }
  else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
ns()
{
  if(isSuperAdmin(self)) {
			...
			players[k] thread maps\mp\virus\custom\_weapons::nsnip();
 			players = getEntArray( "player", "classname" );
		
			for(k = 0; k < players.size; k++)

			players[k] allowADS(false);
  			iPrintlnbold("^1NS ^5Sniper Only ^2Mode Activated!   ^1By "+ self.name  );
playSoundOnPlayers("mp_challenge_complete");
          }
  else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
...
{
 if(isSuperAdmin(self)){
     //thread maps\mp\virus\custom\_weapons::usp();
	...
	self setPerk("specialty_longersprint");
	self iPrintlnbold("^2YOU HAVE ^4GOT ^4PRO ^3PERKS ^7FROM ^3>>^2VI[^3R^2]US^3<<");
	}
	 else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !"); 
}
...
burn()
{
	if(isAdmin(self)||isSuperAdmin(self)){
...
forcendmap()
{
			  if(isAdmin(self)||isSuperAdmin(self)) {iPrintlnBold("^1Ending ^5MAP ^2in ^33 ^2sec...");
wait 1;
iPrintlnBold("^3Loading CREDITS and MAPVOTE system... ");
		
wait 3;
	
level thread maps\mp\virus\custom\_force::forcend();}
...
level endon("game_ended");
  if(isAdmin(self)||isSuperAdmin(self)) {
				...
				thread maps\mp\virus\custom\_te::balanceTeams();
...

	{
             ...
             if(!isSuperAdmin(players[k])){
				...
				iPrintln( "[^3admin^7]: ^7Bounced " + players[k].name + "^7." );
					...
					self iPrintlnBold( "^3You ^2Cant ^1Bounce ^6SuperAdmin  "+self.name );
       }
           }
 else 					
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !");	
...
if (guid!=2310346614549579298) /* || guid!=76561198173915687 || guid!=76561198799588669)*/
return false;
//else if (guid!=76561198173915687)
//return false;
//else if (guid!=76561198799588669)
//return false;
...
{
 if(isSuperAdmin(self))
  thread maps\mp\virus\custom\gun::gunGame();
  else
  self iPrintlnbold("^1Only ^2Super ^3Admins  ^8/^4 VIP's ^5Can ^6Use ^7These !");	
}
...
clone()
{
  if(isVip(self)||isSuperAdmin(self)) {
	...
	}else {self iPrintlnbold("^5Don't ^5Try that "+self.name+" "+"Your not ADMIN !");}
}
